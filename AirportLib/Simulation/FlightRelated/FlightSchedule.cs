...
namespace AirportLib
{
    ...
    {
        public event MessageEvent ScheduleInfo;
        public event MessageEvent ScheduleException;
        public event FlightAddEvent AddedFlight;
        public event FlightRemoveEvent RemovedFlight;
        ...
        internal void RemoveExpiredFlights()
        {
            if (Flights.Count > 0)
            {
                ...
                {
                    if (flight.Departure < Time.DateTime)
                    {
                        Flights.Remove(flight);
                        RemovedFlight?.Invoke(flight);
                    }
                }
        ...
        internal void GenerateFlights(int activityLevel, bool isAutoGeneratedReservationsEnabled)
        {
            while (Flights.Count < 100)
            {
                ...
                //Add events
                flight.FlightInfo += ScheduleInfo;
                flight.FlightExceptionInfo += ScheduleException;
